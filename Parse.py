...
def req():
    return BeautifulSoup(requests.get(url, headers=headers).text, 'lxml').find_all('div', class_='flex_container')[
        1]


def cases():
    return f"""{BeautifulSoup(requests.get(url, headers=headers).text, 'lxml').find_all('div', class_='flex_container')[1].find_all('div', class_='gray_little_statistic')[0].find('strong').text}"""
